        ...
        <div className="max-w-6xl mx-auto px-6 pb-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {jokes.map((joke, index) => (
              <div
                key={joke.id}
                className="group relative bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 hover:bg-white/10 hover:border-purple-500/40 hover:shadow-lg transition-all duration-500 hover:-translate-y-1"
              >
                <div className="absolute -top-3 -left-3 w-10 h-10 bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl flex items-center justify-center text-sm font-bold">
                  #{joke.id}
                </div>
                ...
                {joke.categories.length > 0 && (
                  <div className="flex gap-2 mb-3 justify-end">
                    {joke.categories.map((cat) => (
                      <span
                        key={cat}
                        className="px-3 py-1 text-xs bg-yellow-500/20 text-yellow-300 border rounded-full"
                      >
                        {cat}
                      </span>
                    ))}
          ...
          <div className="flex items-center justify-center gap-4 mt-12">
            <button
              ...
              disabled={page === 1}
              className="px-5 py-2 bg-white/10 rounded-xl disabled:opacity-30"
            >
              ← Prev
            </button>
          ...
          <div className="text-center mt-6">
            <button
              onClick={() =>
                jokeLoad(Math.floor(Math.random() * totalPages) + 1)
              }
              className="px-8 py-3 bg-gradient-to-r from-yellow-500 via-pink-500 to-purple-500 rounded-full"
            >
              Shuffle Jokes
            </button>
          </div>
